...
- [ ] `users/{uid}` bootstrap on first login — **admin-invite only, no self-signup**.
...
| B42 | **`settings/bootstrap` — a one-way latch for the first admin.** Claim the sentinel (only while `claimed == false`, stamping your uid), THEN create your own admin profile, which is permitted only because step one recorded you. | The ORDER is the safety property: a second caller loses at step one and can do nothing at step two. No rule anywhere sets `claimed` back to false. |
| B43 | **The generic `settings/{docId}` rule excludes `bootstrap`.** | Firestore evaluates every matching rule and ORs the results, so the narrower `match /settings/bootstrap` block did NOT override the generic admin-write rule — an admin could flip `claimed` back to false and reopen the one path by which somebody appoints themselves administrator. Found by the new rules tests, not by reading. |
| B39 | **`scripts/bootstrap-admin.js` — the only thing that can mint the first administrator.** | There is no self-signup (D7), every screen is behind `canUseApp`, and only an admin can grant a role — so a freshly deployed project had NOBODY who could let anybody in. `syncClaims` could not fill the gap: it calls `getUserByEmail` and requires the account to already exist. The script creates the Auth user, sets the claims and writes both `users/{uid}` and `usersPublic/{uid}` — miss the claims and they land on "not set up yet"; miss the mirror and the UI shows a raw uid. Prints a password-reset link rather than setting a password, and refuses `--password` against production. |
...
|---|---|---|---|
| **D1** | **Upgrade to the Blaze plan?** Cloud Functions are needed for scheduled rollups, Meta webhooks and server-side claim sync. | Phases 6, 8 | **Yes.** Cost at this scale is negligible and every alternative is worse. Interim: GitHub Action cron (§8.9 option 2). |
...
| ~~D7~~ | ~~Sign-up model~~ | — | **SETTLED**: admin-invite only, implemented. No self-registration route exists; `scripts/syncClaims.js` is the only way to grant a role. |
